...
exports.login = catchAsync(async (req, res, next) => {
  ...
  const user = await User.findOne({ email }).select('+password'); // +<field name> shows us fields with select: false
...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  // 4) Check if user changed password aftre token was issued
  if (currentUser.changedPasswordAfter(decoded.iat)) {
    return next(new AppError('Outdated token. Please log in again.', 401));
  }
...
exports.restrictTo =
  (...roles) =>
  (req, res, next) => {
    // roles is an array, and it's a closure
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  user.passwordResetToken = undefined;
  user.passwordResetExpires = undefined;
  await user.save(); // save() is mongoose function on document
